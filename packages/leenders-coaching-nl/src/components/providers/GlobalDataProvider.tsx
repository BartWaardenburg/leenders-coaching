...
import { getGlobalData } from '@/groq/queries';
import {
  transformNullable,
 ...
 */
export const GlobalDataProvider = async ({
  children,
  ...
  const headerData = {
    ...
    about: {
      ...
      description: transformNullable(
        ...
        defaultHeader.about.description
      ),
    },
    social: {
      title: transformNullable(
        header.social?.title,
        defaultHeader.social.title
      ),
      links:
        transformNullableArray(header.social?.links, (link) => ({
  ...
  const footerData = {
    ...
    contact: {
      email: transformNullable(
        footer.contact?.email,
        defaultFooter.contact.email
      ),
      phone: transformNullable(
        footer.contact?.phone,
        defaultFooter.contact.phone
      ),
    },
    socialLinks:
      transformNullableArray(footer.socialLinks, (link) => ({
        _key: transformNullable(link?._key, ''),
        platform: transformNullable(link?.platform, ''),
        url: transformNullable(link?.url, '#'),
  ...
  const configData: ConfigType = {
    accessibility: {
      closeButtons: {
        toast: transformNullable(
          configuration?.accessibility?.closeButtons?.toast,
          defaultConfig.accessibility.closeButtons.toast
        ),
        modal: transformNullable(
          configuration?.accessibility?.closeButtons?.modal,
          defaultConfig.accessibility.closeButtons.modal
        ),
      },
      calendar: {
        previousMonth: transformNullable(
          configuration?.accessibility?.calendar?.previousMonth,
          defaultConfig.accessibility.calendar.previousMonth
        ),
        nextMonth: transformNullable(
          configuration?.accessibility?.calendar?.nextMonth,
          defaultConfig.accessibility.calendar.nextMonth
        ),
      },
    },
    interface: {
      mobileMenu: {
        toggleButton: transformNullable(
          configuration?.interface?.mobileMenu?.toggleButton,
          defaultConfig.interface.mobileMenu.toggleButton
        ),
        menuLabel: transformNullable(
          configuration?.interface?.mobileMenu?.menuLabel,
          defaultConfig.interface.mobileMenu.menuLabel
        ),
        closeButton: transformNullable(
          configuration?.interface?.mobileMenu?.closeButton,
          defaultConfig.interface.mobileMenu.closeButton
        ),
      },
      themeToggle: {
        label: transformNullable(
          configuration?.interface?.themeToggle?.label,
          defaultConfig.interface.themeToggle.label
        ),
      },
      buttons: {
        loadMore: transformNullable(
          configuration?.interface?.buttons?.loadMore,
          defaultConfig.interface.buttons.loadMore
        ),
        readMore: transformNullable(
          configuration?.interface?.buttons?.readMore,
          defaultConfig.interface.buttons.readMore
        ),
        submit: transformNullable(
          configuration?.interface?.buttons?.submit,
          defaultConfig.interface.buttons.submit
        ),
        close: transformNullable(
          configuration?.interface?.buttons?.close,
          defaultConfig.interface.buttons.close
        ),
      },
    },
    blog: {
      labels: {
        featured: transformNullable(
          configuration?.blog?.labels?.featured,
          defaultConfig.blog.labels.featured
        ),
        readArticle: transformNullable(
          configuration?.blog?.labels?.readArticle,
          defaultConfig.blog.labels.readArticle
        ),
      },
      paths: {
        blog: transformNullable(
          configuration?.blog?.paths?.blog,
          defaultConfig.blog.paths.blog
        ),
      },
    },
    forms: {
      messages: {
        required: transformNullable(
          configuration?.forms?.messages?.required,
          defaultConfig.forms.messages.required
        ),
        invalid: transformNullable(
          configuration?.forms?.messages?.invalid,
          defaultConfig.forms.messages.invalid
        ),
        success: transformNullable(
          configuration?.forms?.messages?.success,
          defaultConfig.forms.messages.success
        ),
        error: transformNullable(
          configuration?.forms?.messages?.error,
          defaultConfig.forms.messages.error
        ),
      },
    },
  };
